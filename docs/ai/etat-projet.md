...
- **Loader :** Fonction asynchrone React Router pour charger les données avant le rendu. Erreurs bloquantes (réseau, auth, serveur, 404…) : **`throwRouteLoaderError`** (`helpers/routeError.ts`) → boundary **`error-page`** ou page **`/erreur`**. Exemple **`detailsSejourLoader`** (route **`sejour-detail`**) : **`Promise.all`** séjour, enfants, groupes, lieux, moments, horaires, activités, types d’activité, **`reunions`**, **`planningGrilles`**, **`activitesPrestataires`** — enregistre le séjour dans **`sejourDetailRouteCache`** pour les loaders enfants. **`menusLoader`** : 2 GET (références alimentaires agrégées + menus sur la plage du séjour), sans 2ᵉ fetch séjour. **`sanitaireLoader`** : dossiers + cahier infirmerie. **`chambresLoader`** : liste **`GET …/chambres`**.
