...
- **Chambres (hébergement)** (`/sejours/{sejourId}/chambres`) : ressource distincte des **Lieux** (activités / surveillance / rassemblements). **`ChambreDto`** : **`typeChambre`** (`ENFANT` | `EQUIPE`), **`identifiant`** (unique par séjour, casse ignorée), **`nom`** optionnel, **`capaciteMax`** (> 0), **`genreAutorise`** (`MASCULIN` | `FEMININ` | `MIXTE`), localisation optionnelle (**`batiment`**, **`couloir`**, **`etage`** — 0 = RDC), **`referents`** (lecture seule ; uniquement **`ENFANT`**). **GET** : **`ACCES_SEJOUR`** ; **POST/PUT/DELETE** chambre et référents : **`GESTION_SEJOURS`** (front : **`peutGererMembresEquipeSejour`**). **Référents** : **`POST …/chambres/{chambreId}/referents`** (body **`AjouterReferentRequest`**, champ **`referentTokenId`**) ; **`DELETE …/referents/{referentTokenId}`**. Passage **ENFANT → EQUIPE** à la modification : l’API supprime les référents. Tri liste API : **`batiment`**, **`etage`**, **`couloir`**, **`identifiant`**. **Hors périmètre API actuel** : affectation des occupants (enfants / membres d’équipe) aux chambres.
...
- **Chambre (séjour) :** Hébergement rattaché au séjour (`ChambreDto`), distinct du **Lieu** (activités). Types **`ENFANT`** (référents animateurs autorisés) et **`EQUIPE`**. UI : onglet **`/mes-sejours/:id/chambres`**, **`ListeChambres.tsx`**, service **`sejour-chambre.service.ts`**. **Affectation occupants** : hors périmètre API actuel.
