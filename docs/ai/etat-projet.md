...
- **Planning organisation (grille) :** Grille de planning **direction** pour un séjour (lignes × jours du séjour, cellules : horaire, moment, groupe, lieu, saisie libre ou membres d’équipe selon `sourceContenuCellules` ; **libellé de ligne** peut aussi être un **membre d’équipe** via `sourceLibelleLignes === "MEMBRE_EQUIPE"` et **`libelleUtilisateurTokenId`** sur chaque ligne). API : **`sejour-planning-grille.service.ts`** ; types **`PlanningGrilleDetailDto`**, **`PlanningGrilleSummaryDto`**, **`PlanningLigneDto`**, **`PlanningCelluleDto`**, requêtes `SavePlanningGrille*`, `SavePlanningLigneRequest`, `PlanningCellulePayload`, etc. dans **`api.d.ts`**. UI : **`ListePlanningsOrganisation`** dans le panneau accordéon **`10`** de **`DetailsSejourOverview`** (props **`grilles`** depuis le loader, **`enumererJoursSejour`**, `sejourId`, `dateDebut` / `dateFin`, équipe + directeur pour **cellules** « membre » et pour **libellés de ligne** « membre »). À la définition **`sourceLibelleLignes`** / **`sourceContenuCellules`**, les types sans entités disponibles sont proposés en options **non sélectionnables** avec indication textuelle (**`noteCreerDabordPourTypePlanning`**) tant qu’aucune valeur enregistrée ne les utilise. Distinct du **calendrier des activités** (`ListeActivitesCalendrier`).
