...
- **Équipe** : lors du `DELETE /sejours/{id}/equipe/{membreTokenId}`, le backend **retire automatiquement** ce membre des **référents** de tous les groupes du séjour (à garder en tête pour l’UI : pas seulement « hors équipe », aussi « plus référent »). **Privilèges** : endpoint réservé à `ROLE_DIRECTION` ; un `BASIC_USER` (animateur, AS, etc.) reçoit **HTTP 403** avec un body extrait par **`adaptAxiosError`** (champ `error` ou `message` — typiquement *« Access Denied »* via **`CustomAccessDeniedHandler`** côté API). L’UI doit **toujours** afficher cette erreur (voir [decisions-architecturales.md](decisions-architecturales.md), section *Suppression — règle de remontée des erreurs*).
