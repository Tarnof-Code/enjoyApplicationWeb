...
- **Chambres (hébergement)** (`/sejours/{sejourId}/chambres`) : ressource distincte des **Lieux**. **`ChambreDto`** : **`typeChambre`**, **`identifiant`**, **`nom`**, **`capaciteMax`**, **`genreAutorise`**, localisation (**`batiment`**, **`couloir`**, **`etage`**), **`groupe`** (**`GroupeResumeDto`**, type **ENFANT** ; `null` = sans restriction), **`referents`**, **`occupants`**. **Lecture et écriture** (CRUD, référents, occupants) : privilège **`ACCES_SEJOUR`** — tout membre rattaché au séjour (directeur, ANIM, AS, SB, AUTRE, ADJOINT) ; hors séjour → **403**. Front : route **`/mes-sejours/:id/chambres`** protégée **`[DIRECTION, BASIC_USER]`** → actions CRUD + affectation pour tout participant (plus de garde **`peutGererMembresEquipeSejour`** sur cet écran). **Référents** : **`POST/DELETE …/referents`** — **ENFANT** only. **Occupants** : **`POST/DELETE …/occupants/enfants`** (unitaire ou batch) ; **`POST/DELETE …/occupants/equipe`**. Règles : un occupant = une chambre max (réaffectation = déplacement) ; genre / capacité / **`groupeId`** / **`numeroLit`** (409 doublon lit). **PUT** : changement **`typeChambre`** efface occupants ; **ENFANT → EQUIPE** efface référents + groupe. Erreurs : **400** métier, **404**, **409** identifiant ou lit. Tri liste : **`batiment`**, **`etage`**, **`couloir`**, **`identifiant`** ; occupants triés **`numeroLit`**, nulls en fin.
...
- **Chambre (séjour) :** Hébergement rattaché au séjour (`ChambreDto`), distinct du **Lieu**. Types **`ENFANT`** / **`EQUIPE`**. **Droits** : **`ACCES_SEJOUR`** pour lecture et écriture (tous les membres du séjour). UI : **`ListeChambres.tsx`**, helpers **`chambreOccupantsUtils.ts`** (**`fusionnerChambreRetourneeDansListe`** après POST affectation).
