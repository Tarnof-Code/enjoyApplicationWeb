...
- **Types TS** : `ActiviteDto` (**`moment`**, **`typeActivite`**, **`lieu`**, **`avertissementLieu`**, **`groupeIds`**), `CreateActiviteRequest` / `UpdateActiviteRequest` (**`lieuId`**, **`momentId`**, **`typeActiviteId`** obligatoire ; **`null`** sur `lieuId` en mise à jour retire le lieu), **`TypeActiviteDto`** / **`SaveTypeActiviteRequest`**, **`MomentDto`** / **`SaveMomentRequest`**, **`HoraireDto`** / **`SaveHoraireRequest`** (libellé format `6h00`–`23h59`), `LieuDto` / `SaveLieuRequest` (**partage**) dans `api.d.ts` — section **Types d’activité** (par séjour) documentée avant **Activités** lorsque pertinent. **`PlanningCelluleDto`** / **`PlanningCellulePayload`** : références cellule en **tableaux** (`horaireIds`, `horaireLibelles`, `momentIds`, `groupeIds`, `lieuIds`, `membreTokenIds`) — voir **`frontend-planning-cellules-multiples.md`** enjoyApi.
