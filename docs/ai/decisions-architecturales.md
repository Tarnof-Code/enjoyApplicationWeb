...
- **React 19 & Compiler :** 
  ...
  - **Exception documentée** : **`SectionReunionsSejour.tsx`** — **`useMemo`** pour **`peutGererEcritures`** (dérivé Redux + props), **`texteCherchableParId`** (map id → texte ODJ + contenu TipTap pour filtres) et **`reunionsAffichees`** ; **`useCallback`** pour **`reinitialiserFiltres`** et **`recharger`** (liste comptes rendus).
