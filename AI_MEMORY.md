...
Rappels essentiels : réponses en français ; paquets avec `pnpm` ; secrets / `.env*` / dumps interdits ; types partagés dans `src/types/api.d.ts`. **Connexion** : **`connexionShouldRevalidate`**, router singleton **`App.tsx`**, pas de **`<Navigate>`** sur **`Connexion`** ; **401** → redirect **`/`** (pas page « Connexion requise ») ; retour post-login via **`cheminApresConnexion.ts`** + **`chargerProfilEtCheminDestinationApresConnexion`**. **Menus séjour** : **`menusLoader`** (2 GET) + cache séjour **`sejourDetailRouteCache`**. **Sorties / prestas** : **`activitesPrestataires`** dans **`detailsSejourLoader`**. **Erreurs routes** : **`routeError.ts`**, **`RedirectToConnexion`**. **Réunions** : **`initialReunions`** depuis le loader parent.
