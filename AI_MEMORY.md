...
Rappels essentiels : réponses en français ; paquets avec `pnpm` ; secrets / `.env*` / dumps interdits ; types partagés dans `src/types/api.d.ts`. **Auth 401** : redirect **`/`** — loaders **`routeError.ts`** + intercepteur global **`caller.service.ts`**. **Impression** : **`src/print/`** — en-tête **Titre — n/N** ; **`PrintDocumentHeader`** = métadonnées utiles seulement (pas de redondance vue/période/rendu déjà choisis à l’écran). **Calendriers print** : modale **Noir et blanc** / **Couleurs** — **`ListeActivites`** + **`DetailsSejourMenus`** (`couleurFondCarteMenuPourTypeRepas`, preset **`menusGrid`**). **Erreurs routes** : **`routeError.ts`**. **Moments — hiérarchie** : `MomentDto.parentId`, helper **`construireArbreMoments.ts`** (`idsEnConflit`, `aplatirMomentsHierarchiquement`, etc.) ; `ListeMoments` accordéons + modale parent/enfants ; conflits animateurs hiérarchiques dans **`listeActivitesPrestatairesCalendrier.ts`** (`ANIMATEUR_DEJA_AFFECTE_CRENEAU` 400).

Mise à jour mémoire : commande **`/maj`** ; détail dans `docs/ai/` ; garder ce fichier entre 10 et 20 lignes. Dernière MAJ : **2026-06-07** (moments hiérarchie + conflits animateurs + accordéons sous-moments).
