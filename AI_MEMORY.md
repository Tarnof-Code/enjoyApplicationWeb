...
**Dernière revue Memory Bank (`#MAJ` 2026-05-06, alignement [`.cursorrules`](.cursorrules)) :** **Git** — **`master`**, **`HEAD`** **`cbe342b`** (**`origin/master`**). **Index stagé (non commit au #MAJ)** — droits séjour (**`src/helpers/peutGererMembresEquipeSejour.ts`** = directeur **ou** **`RoleSejour.ADJOINT`**) : **`Header.tsx`** n’affiche **Paramétrage** que pour les gestionnaires ; **`DetailsSejourParametrage.tsx`** (**`Navigate`** vers vue générale sinon) ; **`DetailsSejourOverview`**, **`Equipe`** / **`ListeEnfants`** / **`ListeGroupes`**, **`DossierEnfant`**, **`DetailsSejourActivites`** + **`ListeActivites*`**, **`listeActivitesUtils`** (**`equipeAvecTokenEnTete`**), **`DetailsSejourMenus`**, **`DetailsSejourOrganisation`** ; **`ListePlanningsOrganisation`** (**`Consulter le planning`**, **`peutGererPlanningStructure`**) ; **`ParametrageAffichageMenus`** (**`modifiable`**) ; **`ListeReferencesAlimentaires*`** (titre + **Ajouter** alignés ) ; **`CalendrierCarteEditionAvecSuppression`** (**`lectureSeule`**, **`.carteBtnLectureSeule`**). *Antérieurement (commit **`cbe342b`**) :* formulaires — **`getApiErrorMessage`**, **`Form.errorMessagePlacement`**, dossier / groupe / import Excel. Journal : [`docs/ai/contexte-actif.md`](docs/ai/contexte-actif.md).
