...
**Dernière revue Memory Bank (alignement [`.cursorrules`](.cursorrules)) :** 2026-05-04 — **Menus & références alimentaires (détail séjour)** : nouvelle sous-route **`/directeur/sejours/:id/menus`** → **`DetailsSejourMenus`** + **`DetailsSejourMenus.module.scss`** (gestion des menus repas du séjour). **Header** directeur : segment **Menus** (`FaUtensils`) entre **Activités** et **Paramétrage**. **`ListeReferencesAlimentaires`** (+ SCSS) : liste / gestion des références alimentaires réutilisables. **`references-alimentaires.service.ts`**, **`sejour-menu.service.ts`** ; helpers **`dateIsoLocal.ts`**, **`optionsReferencesAlimentaires.ts`** ; types **`api.d.ts`** étendus (menus séjour, références alimentaires). **Dossier enfant** : **`DossierEnfant`**, **`DossierEnfantForm`**, composant **`Form`** générique et **`Form.module.scss`** enrichis pour les informations diététiques. **`DetailsSejourParametrage`** : lien ou entrée alignée avec la navigation (menus comme section dédiée). Synthèse du protocole § « AI MEMORY BANK PROTOCOL » et § « Post-commit / Post-push » :
