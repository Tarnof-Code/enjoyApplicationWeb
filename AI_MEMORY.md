...
Rappels essentiels : réponses en français ; paquets avec `pnpm` ; secrets / `.env*` / dumps interdits ; types partagés dans `src/types/api.d.ts`. **Auth 401** : redirect **`/`** — loaders **`routeError.ts`** + intercepteur global **`caller.service.ts`** (échec refresh → logout, URL mémorisée, pas de message Spring brut). **Impression** : **`src/print/`** — en-tête défaut **Titre — n/N** ; opt-in **`runningHeaderLayout: 'title-page-split'`** (**`DossierEnfant`**). **Activités calendrier** : **`ListeActivites`** — modale **Noir et blanc** / **Couleurs**, preset **`activitesCalendarGrid`**. **Erreurs routes** : **`routeError.ts`**.

Mise à jour mémoire : commande **`/maj`** ; détail dans `docs/ai/` ; garder ce fichier entre 10 et 20 lignes. Dernière MAJ : **2026-06-07** (redirect **401** intercepteur Axios).
