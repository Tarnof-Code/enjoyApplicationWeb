...
**Dernière revue Memory Bank (`#MAJ` 2026-05-08, alignement [`.cursorrules`](.cursorrules)) :** **Git** — **`HEAD`** **`a9c2635`** (**`origin/master`** à jour), **working tree** avec travail **Sanitaire** **non commité** au moment de la revue (fichiers listés dans le journal). **Synthèse** — Route **`/mes-sejours/:id/sanitaire`** + segment **Header** **Sanitaire** (`FaNotesMedical`) ; chargement agrégé **`GET /sejours/{sejourId}/dossiers-enfants`** (**`listerDossiersEnfantsSanitaire`**, types **`GroupeResumeDto`**, **`EnfantDossierSanitaireLigneDto`**). **`DetailsSejourSanitaire`** + **`ListeSanitaireDossiers`** : cases colonnes optionnelles (**`localStorage`** **`enjoy.sanitaire.opts.{sejourId}`**), filtrage des lignes si au moins un groupe de colonnes est affiché (présence de données par groupe ; ET entre groupes). **`Liste.tsx`** : prop **`persistFiltersStorageKey`** → filtres **`sessionStorage`** **`enjoy.liste.filters.{clé}`** pour conserver les filtres du tableau au retour depuis le dossier. **`DossierEnfant`** : **`from: 'sanitaire'`** dans **`location.state`** → **Retour** vers **`/mes-sejours/:id/sanitaire`**. Journal : [`docs/ai/contexte-actif.md`](docs/ai/contexte-actif.md). *Révision précédente (2026-05-07) :* édition dossier par section, permissions AS, usages lieux — détail dans le journal du 07/05.
...
- **Commande `#MAJ`** (message utilisateur exact, hors espaces début/fin) : relire **`.cursorrules`**, analyser le dépôt (**`git status`**, **`git diff`**, commits récents, fichiers **`src/`** / services / types / **`App.tsx`** / loaders), puis mettre à jour pivot + fiches **`docs/ai/`** comme une demande explicite de mise à jour Memory Bank ; pas de résumé chat sans écriture fichier ; détail dans le point **4** de **`.cursorrules`**.
