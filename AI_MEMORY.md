...
Rappels essentiels : réponses en français ; paquets avec `pnpm` ; secrets / `.env*` / dumps interdits ; types partagés dans `src/types/api.d.ts`. **Header** : burger **&lt; 1400px** ; **`--site-header-height`**. **Impression** : **`src/print/`** — en-tête répété défaut **Titre — n/N** ; opt-in **`runningHeaderLayout: 'title-page-split'`** (**`DossierEnfant`** uniquement : titre gauche, **n/N** droite). Listes **`Liste`** / **`ListeSanitaireDossiers`** ; cahier **`ListeCahierInfirmerie`** ; fiche **`/mes-sejours/:id/enfants/:enfantId/dossier`**. **Sanitaire** : bascule cahier/dossiers, barre colonnes **OU** + **Tout**. **Tri** : **`trierUtilisateurs.ts`**. **Erreurs routes** : **`routeError.ts`**.

Mise à jour mémoire : commande **`/maj`** ; détail dans `docs/ai/` ; garder ce fichier entre 10 et 20 lignes. Dernière MAJ : **2026-06-06** (impression fiche **`DossierEnfant`**).
