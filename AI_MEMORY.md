...
**Dernière revue Memory Bank (alignement [`.cursorrules`](.cursorrules)) :** 2026-04-29 — détail séjour **trois sections** dans le Header directeur (**Vue générale** / **Activités** / **Paramétrage**) ; route **`/directeur/sejours/:id/parametrage`** (`DetailsSejourParametrage`) pour Lieux / Moments / Horaires / Types d’activité ; vue générale allégée (accordéons **`1`–`4`**, **`10`** uniquement) ; composant partagé **`components/DetailsSejour/DetailsSejourAccordionItem.tsx`** (styles via `Pages/.../DetailsSejour.module.scss`). Synthèse du protocole § « AI MEMORY BANK PROTOCOL » et § « Post-commit / Post-push » :
