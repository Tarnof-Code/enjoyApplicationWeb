...
**Dernière revue Memory Bank (alignement [`.cursorrules`](.cursorrules)) :** 2026-05-04 — **Planning calendrier partagé (premier dépôt de la séquence)** : dossier **`src/components/PlanningCalendrier/`** — **`CalendrierCarteEditionAvecSuppression`** (bouton principal d’édition + suppression latérale), **`CalendrierCelluleAjoutHint`**, **`PlanningModalFooterFormulaire`** (pied de modal avec zone d’erreur et actions), **`calendrierCelluleClavier.ts`** (`declencherActionCalendrierClavier`, `proprietesTdAjoutPlanning` pour `td` « case ajout » clavier Enter/Espace), **`PlanningCalendrier.module.scss`** (dont variable **`--plan-cal-carte-bg`** pour fond de carte). **Intégration** dans **`ListeActivites*`** ou **`DetailsSejourMenus`** : commits distincts suivants (voir journal).
