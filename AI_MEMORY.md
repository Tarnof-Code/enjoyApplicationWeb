...
**Dernière revue Memory Bank (`#MAJ` 2026-05-06, alignement [`.cursorrules`](.cursorrules)) :** **Git** — branche **`master`**, **`HEAD`** = **`ea5f4f1`** (à jour avec **`origin/master`**). **Index stagé (non commit)** : **`src/helpers/axiosError.ts`**, **`Form.tsx`**, **`Form.module.scss`**, **`CreateGroupeForm.tsx`**, **`ImportExcelEnfants.tsx`**, **`Pages/_Sejours/DossierEnfant/DossierEnfant.tsx`** — **`getApiErrorMessage`** ; extraction **`message`** puis **`error`** sur le corps REST ; erreurs loader dossier via **`AdaptedError`** + **`response.data`** ; **`CreateGroupeForm`** avec **`errorMessagePlacement="bottom"`** ; **`Form.errorMessagePlacement`** (`top` \| `bottom`) et **`.errorMessageBelowButtons`**. **Working tree non stagé** : **`.cursorrules`** (point **4** *Commande `#MAJ`*, **`getApiErrorMessage`** / **`Form.errorMessagePlacement`** dans *Qualité* / *Formulaires*), **`AI_MEMORY.md`**, **`docs/ai/contexte-actif.md`**, **`decisions-architecturales.md`**, **`etat-projet.md`** — à indexer avant commit si l’on veut un snapshot repo cohérent. Journal : [`docs/ai/contexte-actif.md`](docs/ai/contexte-actif.md).
