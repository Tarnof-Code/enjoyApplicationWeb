...
- **En cours :** Affinements **planning activités / calendrier** (filtres, UX, perf) — **`ListeActivitesCalendrier.tsx`**. Les **plannings direction** (grilles par séjour) sont gérés dans **`ListePlanningsOrganisation`** (voir *Dernière réalisation*) ; des évolutions plus larges (affectations avancées, vues suppl.) restent sur la roadmap.
...
- **Composants UI :** 
  ...
  - Composants enfants :
    ...
    - `ListePlanningsOrganisation.tsx` : **Plannings organisation** (grilles direction) — cartes par planning, modale tableau éditeur, sections, DnD lignes / sections, **`sejourPlanningGrilleService`** ; voir *Dernière réalisation* et glossaire **Planning organisation (grille)**.
