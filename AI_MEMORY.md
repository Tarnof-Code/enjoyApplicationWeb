...
Rappels essentiels : réponses en français ; paquets avec `pnpm` ; secrets / `.env*` / dumps interdits ; types partagés dans `src/types/api.d.ts`. **Connexion** : **`connexionShouldRevalidate`**, router singleton **`App.tsx`**, pas de **`<Navigate>`** sur **`Connexion`**. **Menus séjour** : **`menusLoader`** (2 GET : refs agrégées + menus) — séjour via **`sejourDetailRouteCache`** (rempli par **`detailsSejourLoader`**), pas de fetch au **`useEffect`** (évite doublon Strict Mode) ; utils **`menusSejourUtils.ts`**. **Références alimentaires** : catalogue **`ListeReferencesAlimentaires`** (param. accordéon **5**) — tri API **`ordre`** via **`trierReferencesAlimentaires`**, champ non affiché (auto à la création). **Erreurs routes** : **`routeError.ts`**, **`throwRouteLoaderError`**. **Réunions** : **`initialReunions`** depuis le loader parent.
