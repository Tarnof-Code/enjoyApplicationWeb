...
**Dernière revue Memory Bank (2026-05-10, alignement [`.cursorrules`](.cursorrules)) :** **Git** — **`HEAD`** **`89f4f4d`** (**`master`** = **`origin/master`** au moment de la revue) ; **working tree** — changements **locaux non commités** sur **historique des modifications** (activités + cellules planning) et rappel post-suppression calendrier (fichiers typiques : **`ListeActivites*.tsx`**, **`ListePlanningsOrganisation*`**, **`HistoriqueModificationListeModal.tsx`**, **`libelleHistoriqueModification.ts`**, **`formaterDate.ts`**, **`sejour-activite.service`**, **`sejour-planning-grille.service`**, **`api.d.ts`**, **`CalendrierCarteEditionAvecSuppression`** — voir `git status`). **Synthèse** — Endpoints **`GET …/historique`** (activités, cellules planning) ; modal partagée **`HistoriqueModificationListeModal`**, snapshots compacts pipe-separated, comparaison lisible (**`libelleHistoriqueModification`**), **`parseDate`** pour estampilles (éviter epoch ms/s confondu), libellé **« Vide »** pour segments vides ; **calendrier activités** : après suppression depuis une case, bandeau **`calendrierCellRappelSuppression`** **dans la cellule** ciblée (**contexte** `animateurTokenId` + `dateYmd` à la demande de suppression), accès **Historique** sur l’`activiteId` conservé. Journal : [`docs/ai/contexte-actif.md`](docs/ai/contexte-actif.md). *Révision précédente (2026-05-08) :* écran **Sanitaire** (`/mes-sejours/:id/sanitaire`), **`GET …/dossiers-enfants`**, **`persistFiltersStorageKey`** / **`Liste`** — détail dans le journal du 08/05.
