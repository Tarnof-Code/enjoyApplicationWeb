...
Rappels essentiels : réponses en français ; paquets avec `pnpm` ; secrets / `.env*` / dumps interdits ; types partagés dans `src/types/api.d.ts`. **Header** : burger sous **1400px** ; **`--site-header-height`**. **Vue générale** : accordéons **fermés** par défaut. **Tri personnes** : `helpers/trierUtilisateurs.ts` — affichage **Prénom Nom** → `trierParPrenomPuisNom` ; **Nom Prénom** → `trierParNomPuisPrenom` / `trierEnfantsParNom`. **Sélection groupes** : `helpers/groupesParType.ts` + **`SelectionGroupesParType`** (sections par type, tri alpha). **Listes** : **`AffichageGroupesListe`**, **`couleurGroupe.ts`**. **Chambres** : **`ListeChambres`**. **Erreurs routes** : **`routeError.ts`**.
