...
**Dernière revue Memory Bank (`#MAJ` 2026-05-07, alignement [`.cursorrules`](.cursorrules)) :** **Git** — **`master`**, **`HEAD`** **`002d590`** (**`origin/master`**), arbre **propre**. **Redirection après authentification** — dernier séjour consulté (**`localStorage`** clé **`enjoy.dernierSejourId.{sub}`**, aligné sur le JWT) enregistré depuis **`Header.tsx`** quand le loader **`sejour-detail`** expose le séjour ; **non effacé** au **`logout`** (seul le contexte « header » reste en **`sessionStorage`** via **`effacerHeaderSejourContext`**). Helpers **`helpers/headerSejourContext.ts`** (**`enregistrerDernierSejourVisite`**, **`lireDernierSejourVisite`**) ; **`helpers/redirectApresAuthentification.ts`** — **`chargerProfilEtCheminAccueil`** (**`utilisateurService.getUser`** puis **`/mes-sejours/:id`** si rôle **`DIRECTION` | `BASIC_USER`** et id connu ; sinon **`/profil`**), **`cheminAccueilDepuisEtatActuel`** pour **`Navigate`**). **`App.tsx`** : loader **`/`** déjà connecté → même logique asynchronous ; **`Connexion.tsx`** : **`loginAction`** + éviter **`/profil`** systématique. **`ADMIN`** (pas de route **`/mes-sejours`**) → **`/profil`**. Journal : [`docs/ai/contexte-actif.md`](docs/ai/contexte-actif.md). *Révision précédente (2026-05-06) :* droits **`peutGererMembresEquipeSejour`** (/**`Header`** **Paramétrage**, écrans associés, formulaires API) — détail historique dans le journal.
