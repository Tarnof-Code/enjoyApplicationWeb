...
Rappels essentiels : réponses en français ; paquets avec `pnpm` ; secrets / `.env*` / dumps interdits ; types partagés dans `src/types/api.d.ts`. **Auth 401** : redirect **`/`**. **Impression** : **`src/print/`** — plannings orga : **`PlanningGrillePrintTable`**, **Impression multiple** (liste **`/organisation`**, ordre ↑↓, sans consignes à l’impression). **Moments** : hiérarchie **`construireArbreMoments.ts`**, conflits **`idsEnConflit`**. **Modale activité** : **`verifierDisponibiliteLieuActivite`** + **`premierConflitAnimateurActiviteInterne`**. **Enfants participants activité** : **`ActiviteEnfantsParticipantsModal`**, **`enfantIds`** / **`ActiviteDto.enfants`**, helpers **`listeActivitesUtils`**.

Mise à jour mémoire : commande **`/maj`** ; détail dans `docs/ai/` ; garder ce fichier entre 10 et 20 lignes. Dernière MAJ : **2026-06-07** (plannings organisation — impression multiple, ordre, UX print).
