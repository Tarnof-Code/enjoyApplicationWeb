...
**Dernière revue Memory Bank (2026-05-17, alignement [`.cursorrules`](.cursorrules)) :** **Git** — **`HEAD`** **`3a97c6e`** ; **index / working tree** avec évolutions **cahier d’infirmerie** (à recouper avec **`git status`** avant commit). **Synthèse** — écran **Sanitaire** : **`ListeCahierInfirmerie`**, **`CahierInfirmerieForm`**, **`cahier-infirmerie.service.ts`**, **`droitsCahierInfirmerie`**, **`cahierInfirmerieLabels.ts`** ; **`dateHeure`** affichée via **`formatDateHeureHistorique`** / **`parseDate`** (timestamps s vs ms) ; historique **`HistoriqueModificationListeModal`** avec **`formatSnapshots="cahier_infi"`** (**`libelleHistoriqueModification`** : fusion Soins + température, « Autre » = libellé **précision seule**, libellé sérum **Serum physiologique**). Détail : [`docs/ai/contexte-actif.md`](docs/ai/contexte-actif.md) ; alignement API : [`docs/ai/etat-projet.md`](docs/ai/etat-projet.md). *Antérieurement (2026-05-11) :* planning **PATCH ma-présence** — [`docs/ai/contexte-actif.md`](docs/ai/contexte-actif.md).
