...
**Dernière revue Memory Bank (alignement [`.cursorrules`](.cursorrules)) :** 2026-04-29 — **Organisation** : route **`/directeur/sejours/:id/organisation`** (liste des grilles) et **`…/organisation/:grilleId`** (éditeur pleine page), **`DetailsSejourOrganisationLayout`** (`Outlet`) + **`DetailsSejourOrganisation`** (`useRouteLoaderData("sejour-detail")`, navigation vers sous-route au lieu de modale tableau XL) ; segment **Organisation** dans le Header directeur (`FaTable`). **Vue générale** : retrait du panneau accordéon **`10`** (plannings organisation uniquement sur la page dédiée) ; accordéons **`1`–`4`** uniquement. **Plannings** : **`ListePlanningsOrganisation`** — fenêtre **1 / 3 / 7 jours** + navigation de période (comme les activités) via hook partagé **`useCalendrierFenetreJours`** + **`CalendrierNavigationPeriode`** ; **`ListeActivites`** utilise le même hook pour la logique fenêtre (réduction duplication). Styles tableau : **`planningGridWindow`**, colonnes Actions / libellé à largeurs fixes, colonne Actions à droite. Synthèse du protocole § « AI MEMORY BANK PROTOCOL » et § « Post-commit / Post-push » :
