...
**Dernière revue Memory Bank (alignement [`.cursorrules`](.cursorrules)) :** 2026-05-04 — Séquence **planning calendrier** : (**1**) dossier **`src/components/PlanningCalendrier/`** — primitives (**`CalendrierCarteEditionAvecSuppression`**, **`CalendrierCelluleAjoutHint`**, **`PlanningModalFooterFormulaire`**, **`calendrierCelluleClavier.ts`**, **`PlanningCalendrier.module.scss`**, dont **`--plan-cal-carte-bg`**). (**2**) **Liste des activités** : migration vers ces primitives — fichiers **`ListeActivites.tsx`**, **`ListeActivitesCalendrier.tsx`**, **`ListeActivites.module.scss`** (imports des composants / styles partagés, même UX calendrier + accessibilité des cases ajout ; détail jour à jour dans [`docs/ai/contexte-actif.md`](docs/ai/contexte-actif.md)). (**3**) **Menus séjour** : intégration **`DetailsSejourMenus*`** peut suivre dans un commit dédié.

**Récapitulatif antérieur (même date, autres livrables) — Menus & références alimentaires (détail séjour)** : sous-route **`/directeur/sejours/:id/menus`** → **`DetailsSejourMenus`** + **`DetailsSejourMenus.module.scss`**. **Header** directeur : segment **Menus** (`FaUtensils`) entre **Activités** et **Paramétrage**. **`ListeReferencesAlimentaires`** (+ SCSS). **`references-alimentaires.service.ts`**, **`sejour-menu.service.ts`** ; helpers **`dateIsoLocal.ts`**, **`optionsReferencesAlimentaires.ts`** ; **`api.d.ts`**. **Dossier enfant** : **`DossierEnfant`**, **`DossierEnfantForm`**, **`Form`** / **`Form.module.scss`** (diététique). **`DetailsSejourParametrage`** aligné navigation.
