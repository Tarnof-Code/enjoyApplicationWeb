...
Rappels essentiels : réponses en français ; paquets avec `pnpm` ; secrets / `.env*` / dumps interdits ; types partagés dans `src/types/api.d.ts`. **Connexion** : **`connexionShouldRevalidate`**, router singleton **`App.tsx`**, pas de **`<Navigate>`** sur **`Connexion`** ; **401** → redirect **`/`** ; retour post-login via **`cheminApresConnexion.ts`**. **Menus séjour** : **`menusLoader`** (2 GET) + cache séjour **`sejourDetailRouteCache`**. **Chambres** : route **`/mes-sejours/:id/chambres`**, **`chambresLoader`**, **`ListeChambres`** (CRUD + référents **ENFANT** ; affectation occupants hors périmètre API). **Sorties / prestas** : **`activitesPrestataires`** dans **`detailsSejourLoader`**. **Erreurs routes** : **`routeError.ts`**, **`RedirectToConnexion`**. **Réunions** : **`initialReunions`** depuis le loader parent.
