...
Rappels essentiels : réponses en français ; paquets avec `pnpm` ; secrets / `.env*` / dumps interdits ; types partagés dans `src/types/api.d.ts`. **Header** : burger **&lt; 1400px** ; **`--site-header-height`** ; lien **Profil** = `Prénom (rôle)` via **`libelleRoleBadgeProfil`**. **Rôle affiché** : séjour en contexte (**`headerSejourContext`**) → rôle sur le séjour (**`libelleRoleSurSejour.ts`**) ; sinon rôle système ; **ADMIN** → **Admin**. **Profil** : carte circulaire + badge rôle (même logique). **Vue générale** : accordéons **fermés**. **Impression** : **`src/print/`** — en-tête répété **Titre — n/N** (**13pt gras**) ; **`PrintDocumentHeader`** = métadonnées seules ; listes **`ListeEnfants`** / **`TableauUtilisateurs`** / **`ListeSanitaireDossiers`** (**`canPrint`** via **`Liste`**, **`ListePrintTable`**, `vertical-align: middle` en-têtes + cellules) ; **cahier** **`ListeCahierInfirmerie`** (filtres jour/recherche, **Signature de la direction**) ; plannings **`ListePlanningsOrganisation`** (fenêtre **1/3/7 j**). Marge bas **20mm** ; **`ignoreGlobalStyles`** pour listes. **Sanitaire — dossiers** : barre **`optionsBar`** (colonnes **OU**, case **Tout**) + impression adaptée aux cases/filtres ; colonne **Groupe(s)** étroite (**`enjoy-sanitaire-col-groupes`**). **Tri** : **`trierUtilisateurs.ts`** ; groupes : **`SelectionGroupesParType`**. **Erreurs routes** : **`routeError.ts`**.

Mise à jour mémoire : commande **`/maj`** (`.cursor/commands/maj.md`) ou règle `.cursor/rules/10-memory-bank.mdc` ; détail dans `docs/ai/` ; garder ce fichier entre 10 et 20 lignes. Dernière MAJ : **2026-06-06** (impression dossiers sanitaires + styles print listes).
