...
Rappels essentiels : réponses en français ; paquets avec `pnpm` ; secrets / `.env*` / dumps interdits ; types partagés dans `src/types/api.d.ts`. **Connexion** : **`connexionShouldRevalidate`**, router singleton **`App.tsx`**, pas de **`<Navigate>`** sur **`Connexion`**. **Menus séjour** : **`menusLoader`** (2 GET) + cache séjour **`sejourDetailRouteCache`**. **Sorties / prestas** : **`activitesPrestataires`** dans **`detailsSejourLoader`** ; page **`…/activites/sorties-prestas`** ; calendrier **`ListeActivites`** (conflits, **`nonParticipations`**) ; helpers **`listeActivitesPrestatairesCalendrier.ts`**. **Erreurs routes** : **`routeError.ts`**, **`throwRouteLoaderError`**. **Réunions** : **`initialReunions`** depuis le loader parent.
